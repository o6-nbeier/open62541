 ...
 *
 * In-process handshake test for the ECC_nistP256_ChaChaPoly SecurityPolicy
...
static void setup(void) {
     ...
     * the ECC_nistP256_ChaChaPoly policy so the password is carried in an
     * EccEncryptedSecret (exercises the ECC user-token decrypt path). */
    UA_String utpUri = UA_STRING(CHACHAPOLY_URI);
...
static UA_Client *
createChaChaPolyClient(UA_MessageSecurityMode mode) {
    UA_ByteString certificate;
    certificate.length = CERT_P256_DER_LENGTH;
    certificate.data = CERT_P256_DER_DATA;
    ...
    cc->securityPolicyUri = UA_STRING_ALLOC(CHACHAPOLY_URI);
...
START_TEST(chachapoly_connect_sign) {
    UA_Client *client = createChaChaPolyClient(UA_MESSAGESECURITYMODE_SIGN);
...
/* SignAndEncrypt mode */
START_TEST(chachapoly_connect_signandencrypt) {
    UA_Client *client = createChaChaPolyClient(UA_MESSAGESECURITYMODE_SIGNANDENCRYPT);
    ck_assert(client != NULL);
...
START_TEST(chachapoly_connect_username) {
    UA_Client *client = createChaChaPolyClient(UA_MESSAGESECURITYMODE_SIGNANDENCRYPT);
    ck_assert(client != NULL);
    UA_ClientConfig *cc = UA_Client_getConfig(client);
 ...
 * bound to the ChaChaPoly policy. Exercises signUserTokenSignature (client) and
 * checkActivateSessionX509 (server) under secureChannelEnhancements. */
START_TEST(chachapoly_connect_certificate) {
    UA_Client *client = createChaChaPolyClient(UA_MESSAGESECURITYMODE_SIGNANDENCRYPT);
    ck_assert(client != NULL);
    UA_ClientConfig *cc = UA_Client_getConfig(client);
...
START_TEST(chachapoly_renew_multiple) {
    UA_Client *client = createChaChaPolyClient(UA_MESSAGESECURITYMODE_SIGNANDENCRYPT);
    ck_assert(client != NULL);
...
START_TEST(chachapoly_renew_token_overlap) {
    UA_Client *client = createChaChaPolyClient(UA_MESSAGESECURITYMODE_SIGNANDENCRYPT);
    ck_assert(client != NULL);
...
static Suite* testSuite_encryption(void) {
    Suite *s = suite_create("EncryptionChaChaPoly");
    TCase *tc = tcase_create("Encryption ECC_nistP256_ChaChaPoly");
    tcase_add_checked_fixture(tc, setup, teardown);
#ifdef UA_ENABLE_ENCRYPTION
    tcase_add_test(tc, chachapoly_connect_sign);
    tcase_add_test(tc, chachapoly_connect_signandencrypt);
    tcase_add_test(tc, chachapoly_connect_username);
    tcase_add_test(tc, chachapoly_connect_certificate);
    tcase_add_test(tc, chachapoly_renew_multiple);
    tcase_add_test(tc, chachapoly_renew_token_overlap);
#endif
