 ...
 *
 * Mirrors check_eventloop_tcp.c, but drives the TCP ConnectionManager with
 * the GLib-backed EventLoop (UA_EventLoop_new_GLib) instead of the select/
 * epoll-backed UA_EventLoop_new_POSIX. Both backends share the same TCP
 * ConnectionManager implementation (arch/posix/eventloop_posix_tcp.c), so
 * the test bodies are identical -- only setupEL() differs. */
...
static void setupEL(void) {
    /* NULL -> attach to the process-wide default GMainContext */
...
START_TEST(staticSendBuffer) {
     ...
     * configured send-bufsize is untouched. */
...
int main(void) {
    Suite *s  = suite_create("Test TCP EventLoop (GLib)");
